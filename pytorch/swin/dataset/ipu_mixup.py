...
class Mixup:
    ...
    def __init__(
        ...
        if self.cutmix_minmax is not None:
            ...
            # force cutmix alpha == 1.0 when minmax active to keep logic simple
            # & safe
    ...
    def _mix_batch(self, x):
        ...
        if use_cutmix:
            (yl, yh, xl, xh), lam = cutmix_bbox_and_lam(x.shape, lam,
                                                        ratio_minmax=self.cutmix_minmax, correct_lam=self.correct_lam)
        ...
        for i in range(num_elem):
            ...
            if lam != 1.:
                if use_cutmix[i]:
                    if not half:
                        mixed = mixed.copy()
                    (yl, yh, xl, xh), lam = cutmix_bbox_and_lam(output.shape, lam,
                                                                ratio_minmax=self.cutmix_minmax, correct_lam=self.correct_lam)
                    mixed[:, yl:yh, xl:xh] = batch[j][0][:, yl:yh, xl:xh]
                    lam_batch[i] = lam
                else:
                    mixed = mixed.astype(np.float32) * lam + \
                        batch[j][0].astype(np.float32) * (1 - lam)
            ...
            if lam < 1.:
                if use_cutmix[i]:
                    ...
                    mixed_temp = mixed_i.astype(
                        np.float32) * lam + mixed_j.astype(np.float32) * (1 - lam)
                    mixed_j = mixed_j.astype(
                        np.float32) * lam + mixed_i.astype(np.float32) * (1 - lam)
                    mixed_i = mixed_temp
