...
def adapt_input_conv(in_chans, conv_weight):
    ...
    if in_chans == 1:
        ...
        if I != 3:
            ...
            # the original RGB input layer weights that'd work better for
            # specific cases.
...
def load_pretrained(
    ...
    state_dict = load_state_dict_from_url(
        cfg['url'], progress=progress, map_location='cpu')
    ...
    if num_classes != cfg['num_classes']:
        ...
        # special case for pretrained weights with an extra background class in
        # pretrained weights
        classifier_weight = state_dict[classifier_name + '.weight']
        state_dict[classifier_name +
                   '.weight'] = classifier_weight[label_offset:]
...
def adapt_model_from_string(parent_module, model_string):
    ...
    for n, m in parent_module.named_modules():
        ...
        if isinstance(old_module, nn.Linear):
            # FIXME extra checks to ensure this is actually the FC classifier
            # layer and not a diff Linear layer?
...
def default_cfg_for_features(default_cfg):
    ...
    # remove default pretrained cfg fields that don't have much relevance for
    # feature backbone
    # add default final pool size?
    to_remove = ('num_classes', 'crop_pct', 'classifier')
    for tr in to_remove:
...
def build_model_with_cfg(
    ...
    # for classification models, check class attr, then kwargs, then default
    # to 1k, otherwise 0 for feats
    ...
    if features:
        ...
        model.default_cfg = default_cfg_for_features(
            default_cfg)  # add back default_cfg
...
def model_parameters(model, exclude_head=False):
    if exclude_head:
        # FIXME this a bit of a quick and dirty hack to skip classifier head
        # params based on ordering
