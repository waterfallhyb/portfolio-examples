...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = x[:, torch.arange(1, H, 2), 0::2, :]  # B H/2 W/2 C
        x2 = x[:, 0::2, torch.arange(1, W, 2), :]  # B H/2 W/2 C
        x3 = x[:, torch.arange(1, H, 2), :, :]  # B H/2 W/2 C
        x3 = x3[:, :, torch.arange(1, W, 2), :]
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        if target is None:
            ...
            return x, poptorch.identity_loss(self.celoss(x, target), 'none')
